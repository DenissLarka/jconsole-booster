...
public class XTree extends JTree {
	...
	private static String getKeyPropertyListString(ObjectName mbean) {
		...
		orderedKeyPropertyListString = orderedKeyPropertyListString.substring(0,
				orderedKeyPropertyListString.length() - 1);
		...
		private void addMBeanInfoNodes(DefaultTreeModel tree, DefaultMutableTreeNode node, XMBean mbean,
			...
			if (ai != null && ai.length > 0) {
				...
				for (MBeanAttributeInfo mbai : ai) {
					...
					XNodeInfo attributeUO = new XNodeInfo(XNodeInfo.Type.ATTRIBUTE, new Object[]{mbean, mbai},
							mbai.getName(), null);
			...
			if (oi != null && oi.length > 0) {
				...
				for (MBeanOperationInfo mboi : oi) {
					...
					XNodeInfo operationUO = new XNodeInfo(XNodeInfo.Type.OPERATION, new Object[]{mbean, mboi},
							mboi.getName(), toolTipText);
			...
			if (isBroadcaster != null && isBroadcaster.booleanValue()) {
				...
				XNodeInfo notificationsUO = new XNodeInfo(XNodeInfo.Type.NOTIFICATIONS, mbean, Messages.NOTIFICATIONS,
						null);
				...
				if (ni != null && ni.length > 0) {
					for (MBeanNotificationInfo mbni : ni) {
						...
						XNodeInfo notificationUO = new XNodeInfo(XNodeInfo.Type.NOTIFICATION, mbni, mbni.getName(),
								null);
