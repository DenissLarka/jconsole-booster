	...
	public static Component loadArray(Object value) {
		...
		if (isViewableValue(value)) {
			...
			for (int i = 0; i < arr.length; i++) {
				if (i % 2 == 0) {
					sb.append("<tr style=\"background-color: ").append(evenRowColorStr).append("\"><td><pre>")
							.append(arr[i] == null ? arr[i] : htmlize(arr[i].toString())).append("</pre></td></tr>");
				} else {
					sb.append("<tr style=\"background-color: ").append(oddRowColorStr).append("\"><td><pre>")
							.append(arr[i] == null ? arr[i] : htmlize(arr[i].toString())).append("</pre></td></tr>");
				}
			}
			if (arr.length == 0) {
