...
public class TableSorter extends DefaultTableModel implements MouseListener {
    ...
    private void sort(int column, boolean isAscending) {
        ...
        if (JConsole.isDebug()) {
            logger.debug("sorting table against column=" + column + " ascending=" + isAscending);
        }
        quickSort(0, getRowCount() - 1, column, isAscending);
        // restore registered listeners
        restoreListeners();

        // update row heights in RowHeightSink (required by expandable cells)
        final RowHeightSink sink = (tableView instanceof RowHeightSink) ? (RowHeightSink) tableView : null;
        if (sink != null) {
            for (int i = 0; i < getRowCount(); i++) {
                Vector<?> data = dataVector.elementAt(i);
                sink.updateRowHeight(data.elementAt(1), i);
            }
        }
    }
