...
/**
 * Wraps {@link SampleMarkup} so that JMX introspection picks up markup-bearing descriptions for parameters and
 * operations. Without this wrapper a plain Standard MBean would surface descriptions like {@code "p1"}, {@code "p2"},
...
public class SampleMarkupStandardMBean extends StandardMBean {
    ...
    static {
        PARAM_DESCRIPTIONS.put("setCurrencyPair", new String[] {"Currency pair {{combo:EURUSD,USDCHF,GBPUSD,USDJPY}}"});
        ...
        PARAM_DESCRIPTIONS.put("placeOrder", new String[] {
        ...
        PARAM_DESCRIPTIONS.put("scheduleReport", new String[] {
            "Report name",
            "From {{date:yyyy-MM-dd}}",
            "To {{date:yyyy-MM-dd}}",
            "Output format {{combo:CSV,JSON,XLSX,PDF}}"
        });
        PARAM_DESCRIPTIONS.put("searchTrades", new String[] {
            "FX pair {{combo:EURUSD,USDCHF,GBPUSD,USDJPY,AUDUSD,NZDUSD}}",
            "Side {{combo:BUY,SELL,BOTH}}",
            "Min quantity",
            "Max quantity"
        });
        PARAM_DESCRIPTIONS.put("transferFunds", new String[] {
            "Source account",
            "Destination account",
