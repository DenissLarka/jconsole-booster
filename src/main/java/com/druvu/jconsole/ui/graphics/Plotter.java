...
package com.druvu.jconsole.ui.graphics;
...
public class Plotter extends JComponent implements Accessible, ActionListener, PropertyChangeListener {
    ...
    public static final int[] rangeValues = {
        1,
        5,
        10,
        30,
    ...
    public static final Color defaultColor = Color.blue.darker();
        ...
        try {
            ...
            if (seqs.size() > 0 && seqs.get(0).size > 0) {
                for (int i = 0; i < seqs.get(0).size; i++) {
                    double excelTime = Formatter.toExcelTime(times.time(i));
            ...
            JOptionPane.showMessageDialog(
                    this, Resources.format(Messages.FILE_CHOOSER_SAVED_FILE, file.getAbsolutePath(), file.length()));
    ...
    private String getSizeString(long v, long vMax) {
        ...
        if (unit == Unit.BYTES && decimals == 0) {
            s = Formatter.formatBytes(v, vMax);
        } else {
    ...
    private static synchronized Stroke getDashedStroke() {
        if (dashedStroke == null) {
            dashedStroke = new BasicStroke(
                    1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, new float[] {2.0f, 3.0f}, 0.0f);
        }
        return dashedStroke;
    }
    ...
    public long getLastValue(String key) {
        Sequence seq = getSequence(key);
