...
package com.druvu.jconsole.inspector.attributes;
...
import com.druvu.jconsole.inspector.table.RowHeightSink;
import com.druvu.jconsole.inspector.table.XTable;
import com.druvu.jconsole.inspector.table.XTextFieldEditor;
...
public class XMBeanAttributes extends XTable implements RowHeightSink {
    ...
    public void updateRowHeight(Object obj, int row) {
    ...
    public void setValueAt(Object value, int row, int column) {
        if (!isCellError(row, column)
            ...
            if (JConsole.isDebug()) {
                logger.debug("validating [row=" + row + ", column=" + column + "]: " + getValueName(row) + "=" + value);
            }
    ...
    private Runnable doLoadAttributes(final XMBean mbean, MBeanInfo infoOrNull) throws JMException, IOException {
        ...
        try {
            ...
            for (int i = 0; i < attrsInfo.length; i++) {
                ...
                if (attributeInfo.isReadable()) {
                    unavailableAttrs.put(
                            attributeInfo.getName(), Utils.getActualException(e).toString());
                }
            }
        }
        // end of retrieval
    ...
    public void collapse(final Component c) {
    ...
    //
    @Override
    public void sortRequested(int column) {
        if (isEditing()) stopCellEditing();
    ...
    class AttributesListener implements TableModelListener {
        ...
        public void tableChanged(final TableModelEvent e) {
            ...
            if (isColumnEditable(e.getColumn())) {
                ...
                if (logger.isTraceEnabled()) {
                    logger.trace(
                            "tableChanged: firstRow={}, lastRow={}, column={}, value={}",
                            e.getFirstRow(),
                            e.getLastRow(),
                            e.getColumn(),
                            tableValue);
                ...
                if (tableValue instanceof String) {
                    try {
                        tableValue = Utils.createObjectFromString(
                                getClassName(e.getFirstRow()), // type
