...
public class XMBeanNotifications extends JTable implements NotificationListener {
	...
	private static final String[] columnNames = {Messages.TIME_STAMP, Messages.TYPE, Messages.USER_DATA,
	...
	public synchronized void registerListener(DefaultMutableTreeNode node)
		...
		if (!subscribed) {
			try {
				mbean.getMBeanServerConnection().addNotificationListener(MBeanServerDelegate.DELEGATE_NAME, this, null,
						null);
				subscribed = true;
			} catch (Exception e) {
				if (JConsole.isDebug()) {
				...
				public void run() {
					synchronized (XMBeanNotificationsListener.this) {
						try {
							...
							notifications.fireNotificationReceived(XMBeanNotificationsListener.this, xmbean, node,
									rowData, received);
						} catch (Exception e) {
							if (JConsole.isDebug()) {
