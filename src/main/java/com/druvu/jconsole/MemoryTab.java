...
class MemoryTab extends Tab implements ActionListener, ItemListener {
	...
	public MemoryTab(VMPanel vmPanel) {
		...
		controlPanel.add(new LabeledComponent(Messages.CHART_COLON, Resources.getMnemonicInt(Messages.CHART_COLON),
				plotterChoice));
				...
				for (int i = 0; i < n; i++) {
					...
					try {
						if (plotter instanceof PoolPlotter) {
							...
							AttributeList al = proxyClient.getAttributes(objectName,
	...
	private String formatDetails() {
		...
		text += Formatter.newRow(Messages.USED, Formatter.formatKBytes(used));
		if (committed > 0L) {
			text += Formatter.newRow(Messages.COMMITTED, Formatter.formatKBytes(committed));
		}
		if (max > 0L) {
			text += Formatter.newRow(Messages.MAX, Formatter.formatKBytes(max));
		}
		if (threshold > 0L) {
			text += Formatter.newRow(Messages.USAGE_THRESHOLD, Formatter.formatKBytes(threshold));
		}
		...
		try {
			...
			for (GarbageCollectorMXBean garbageCollectorMBean : garbageCollectors) {
				...
				String str = Resources.format(Messages.GC_TIME_DETAILS,
						Formatter.justify(Formatter.formatTime(gcTime), 14), gcName, String.format("%,d", gcCount));
				if (!descPrinted) {
					text += Formatter.newRow(Messages.GC_TIME, str);
					descPrinted = true;
				} else {
					text += Formatter.newRow(null, str);
				}
			}
		} catch (IOException e) {
		}
		...
		protected class AccessiblePoolChart extends AccessibleJPanel {
			public String getAccessibleName() {
				...
				for (PoolPlotter poolPlotter : poolPlotters) {
					...
					keyValueList += Resources.format(Messages.PLOTTER_ACCESSIBLE_NAME_KEY_AND_VALUE,
	...
	private static class MemoryOverviewPanel extends OverviewPanel {
		...
		private void updateMemoryInfo(long used, long committed, long max) {
			getInfoLabel()
					.setText(Resources.format(Messages.MEMORY_TAB_INFO_LABEL_FORMAT, Formatter.formatBytes(used, true),
							Formatter.formatBytes(committed, true), Formatter.formatBytes(max, true)));
		}
	}
}
