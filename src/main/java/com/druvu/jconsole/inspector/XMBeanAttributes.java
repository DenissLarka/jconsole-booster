...
public class XMBeanAttributes extends XTable {
	...
	private Runnable doLoadAttributes(final XMBean mbean, MBeanInfo infoOrNull) throws JMException, IOException {
		...
		try {
			...
			if (JConsole.isDebug()) {
				logger.error("Error calling getAttributes() on MBean \"" + mbean.getObjectName() + "\". JConsole will "
						+ "try to get them individually calling " + "getAttribute() instead.", e);
			}
				...
				for (int i = 0; i < attrsInfo.length; i++) {
					...
					if (!attrs.containsKey(attributeInfo.getName())
						...
						if (attributeInfo.isReadable()) {
							...
							// went wrong.
							try {
								Object v = mbean.getMBeanServerConnection().getAttribute(mbean.getObjectName(),
		...
		synchronized (viewersCache) {
			...
			if (viewers.containsKey(attribute)) {
				...
				if (cell.isMaximized() && cell.getType() != XDataViewer.NUMERIC) {
					...
					Component comp = mbeansTab.getDataViewer().createAttributeViewer(value, mbean, attribute,
							XMBeanAttributes.this);
	...
	class AttributesListener implements TableModelListener {
		...
		public void tableChanged(final TableModelEvent e) {
			...
			if (isColumnEditable(e.getColumn())) {
				...
				if (logger.isTraceEnabled()) {
					logger.trace("tableChanged: firstRow={}, lastRow={}, column={}, value={}", e.getFirstRow(),
							e.getLastRow(), e.getColumn(), tableValue);
				}
